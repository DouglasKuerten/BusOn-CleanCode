...
const obterTodasInstituicoes = async (req, res, next) => {
  try {
    const instituicoes = await InstituicaoService.obterTodasInstituicoes(
      req.query,
    );
...
const criarInstituicao = async (req, res, next) => {
  try {
    const novaInstituicao = await InstituicaoService.criarInstituicao(
      req.body,
      req.file,
    );
...
const atualizarInstituicao = async (req, res, next) => {
  try {
    const { id } = req.params;
    const instituicaoAtualizada = await InstituicaoService.atualizarInstituicao(
      id,
      req.body,
      req.file,
    );

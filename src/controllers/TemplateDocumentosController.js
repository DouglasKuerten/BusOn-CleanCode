...
const criarTemplateDocumento = async (req, res, next) => {
  try {
    const novoTemplate = await TemplateDocumentoService.criarTemplateDocumento(
      req.body,
    );
...
const obterTodosTemplatesDocumentos = async (req, res, next) => {
  try {
    const templates =
      await TemplateDocumentoService.obterTodosTemplatesDocumentos();
...
const obterTemplateDocumentoPorId = async (req, res, next) => {
  try {
    const { id } = req.params;
    const template =
      await TemplateDocumentoService.obterTemplateDocumentoPorId(id);
...
const atualizarTemplateDocumento = async (req, res, next) => {
  try {
    const { id } = req.params;
    const template = await TemplateDocumentoService.atualizarTemplateDocumento(
      id,
      req.body,
    );
    res.status(StatusCodes.OK).json(template);

...
async function atualizarPagamentoComoAtrasado(
  pagamento,
  parametro,
  dataHojeUTC,
) {
  ...
  const dataComTolerancia = calcularDataComTolerancia(
    pagamento.dataVencimento,
    parametro.diasToleranciaMulta,
  );

  if (dataComTolerancia < dataHojeUTC) {
    const diasAtraso = calcularDiferencaDias(
      dataHojeUTC.getTime(),
      dataComTolerancia.getTime(),
    );
...
async function processarPagamentosDaAssociacao(parametro, dataHojeUTC) {
  const pagamentos = await Pagamento.findAll({
    ...
    where: {
      situacao: {
        [Op.or]: [SituacaoPagamentoEnum.ABERTO, SituacaoPagamentoEnum.ATRASADO],
      },

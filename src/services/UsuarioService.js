...
} from '../validators/UsuarioSchema.js';
...
class UsuarioService {
  ...
  async criarUsuario(dados, foto) {
    ...
    const hashedPassword = await bcrypt.hash(
      dadosValidados.senha || dadosValidados.telefone,
      15,
    );
    ...
    return await Usuario.create({
      ...
      exigirRedefinicaoSenha:
        dadosValidados.exigirRedefinicaoSenha || !dadosValidados.senha,
  ...
  async obterUsuariosCompleto(filters, orderBy) {
    ...
    return usuarios.map((usuario) => {
      ...
      if (usuario.associacao && usuario.associacao.parametro) {
        ...
        valorMensalidade = usuario.associacao.parametro?.[key];
        if (!valorMensalidade) {
          valorMensalidade = 0;
        }
      }

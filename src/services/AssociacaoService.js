...
class AssociacaoService {
  ...
  async criarAssociacao(associacaoData, files) {
    ...
    const logoDeclaracaoUrl = files?.logoDeclaracao
      ? files.logoDeclaracao[0].filename
      : null;
  ...
  async atualizarAssociacao(id, associacaoData, files) {
    ...
    const logoUrl = files?.logo
      ...
      : associacaoExistente.logoUrl;
    const logoDeclaracaoUrl = files?.logoDeclaracao
      ? files.logoDeclaracao[0].filename
      : associacaoExistente.logoDeclaracaoUrl;

    const [updated] = await Associacao.update(
      { ...associacaoData, logoUrl, logoDeclaracaoUrl },
      { where: { id } },
    );

...
describe('AutenticacaoService', () => {
  ...
  describe('login', () => {
    test('deve realizar login com sucesso', async () => {
      ...
      jest.spyOn(Usuario, 'findOne').mockResolvedValue(usuarioFake);

      jest
        .spyOn(TokenAutenticacao, 'criarToken')
    ...
    test('deve lançar erro quando usuário não é encontrado', async () => {
      ...
      await expect(AutenticacaoService.login(loginData)).rejects.toEqual(
        new BusonException(
  ...
  describe('refreshToken', () => {
    test('deve renovar token com sucesso', async () => {
      ...
      jest.spyOn(TokenAutenticacao, 'findOne').mockResolvedValue(storedToken);

      jest
        .spyOn(TokenAutenticacao, 'verificarDataValidade')
    ...
    test('deve lançar erro quando refresh token não é encontrado', async () => {
      ...
      jest.spyOn(TokenAutenticacao, 'findOne').mockResolvedValue(null);

      await expect(
        AutenticacaoService.refreshToken(refreshTokenData),
      ).rejects.toEqual(
        new BusonException(StatusCodes.UNAUTHORIZED, 'Invalid refresh token'),
      );
    });
    ...
    test('deve lançar erro quando refresh token está expirado', async () => {
      ...
      jest.spyOn(TokenAutenticacao, 'destroy').mockResolvedValue(1);

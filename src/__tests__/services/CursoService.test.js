...
describe('CursoService', () => {
    test('obterCursoPorId retorna curso quando encontrado', async () => {
        ...
        jest.spyOn(Curso, 'findByPk')
            .mockResolvedValue(cursoFake);

        await expect(CursoService.obterCursoPorId(1))
            .resolves
            .toEqual(cursoFake);
    });
    ...
    test('validarInstituicao passa quando instituição existe', async () => {
        ...
        await expect(CursoService.validarInstituicao(1))
            .resolves
            .not
            .toThrow();
    });
    ...
    test('validarInstituicao lança erro quando instituição não existe', async () => {
        ...
        await expect(CursoService.validarInstituicao(999))
            .rejects
            .toEqual(expect.objectContaining({
    ...
    test('criarCurso cria novo curso com sucesso', async () => {
        ...
        jest.spyOn(Curso, 'create')
            .mockResolvedValue(cursoCriado);

        await expect(CursoService.criarCurso(novoCurso))
            .resolves
            .toEqual(cursoCriado);

        expect(Curso.create)
            .toHaveBeenCalledWith(novoCurso);
    });
    ...
    test('atualizarCurso atualiza curso existente', async () => {
        ...
        jest.spyOn(Curso, 'update')
            .mockResolvedValue([1]);

        jest.spyOn(Curso, 'findByPk')
            .mockResolvedValue({ id, ...dadosAtualizacao });

        await expect(CursoService.atualizarCurso(id, dadosAtualizacao))
            .resolves
            .toEqual(expect.objectContaining(dadosAtualizacao));
    ...
    test('excluirCurso remove curso com sucesso', async () => {
        jest.spyOn(Curso, 'destroy')
            .mockResolvedValue(1);

        await expect(CursoService.excluirCurso(1))
            .resolves
            .not
            .toThrow();

        expect(Curso.destroy)
    ...
    test('excluirCurso lança erro quando curso não encontrado', async () => {
        ...
        await expect(CursoService.excluirCurso(999))
            .rejects
            .toEqual(expect.objectContaining({
                statusCode: StatusCodes.NOT_FOUND,
                message: 'Curso não encontrado.'

...
describe('InstituicaoService', () => {
    test('obterInstituicaoPorId retorna instituição quando encontrada', async () => {
        ...
        jest.spyOn(Instituicao, 'findByPk')
            .mockResolvedValue(instituicaoFake);

        await expect(InstituicaoService.obterInstituicaoPorId(1))
            .resolves
    ...
    test('criarInstituicao cria nova instituição com logo', async () => {
        ...
        jest.spyOn(Instituicao, 'create')
            .mockResolvedValue(instituicaoCriada);

        await expect(InstituicaoService.criarInstituicao(novaInstituicao, arquivo))
            .resolves
            .toEqual(instituicaoCriada);

        expect(Instituicao.create)
    ...
    test('atualizarInstituicao atualiza instituição existente com nova logo', async () => {
        ...
        jest.spyOn(path, 'join')
            .mockReturnValue('/caminho/para/logo_antiga.png');

        await expect(InstituicaoService.atualizarInstituicao(id, dadosAtualizacao, arquivo))
            .resolves
            .toEqual(instituicaoAtualizada);

        expect(fs.unlink)
            .toHaveBeenCalledWith('/caminho/para/logo_antiga.png');

        expect(Instituicao.update)
    ...
    test('excluirInstituicao remove instituição e logo', async () => {
        ...
        jest.spyOn(Instituicao, 'destroy')
            .mockResolvedValue(1);
        ...
        jest.spyOn(path, 'join')
            .mockReturnValue('/caminho/para/logo.png');

        await expect(InstituicaoService.excluirInstituicao(1))
            .resolves
            .not
            .toThrow();

        expect(fs.unlink)
            .toHaveBeenCalledWith('/caminho/para/logo.png');

        expect(Instituicao.destroy)
    ...
    test('excluirInstituicao lança erro quando instituição não encontrada', async () => {
        ...
        await expect(InstituicaoService.excluirInstituicao(999))
            .rejects
            .toEqual(expect.objectContaining({
                statusCode: StatusCodes.NOT_FOUND,
                message: 'Instituição não encontrada.'

...
describe('PagamentoService', () => {
    ...
    test('obterPagamentoPorId retorna pagamento quando encontrado', async () => {
        jest.spyOn(Pagamento, 'findByPk')
            .mockResolvedValue(pagamentoFake);

        await expect(PagamentoService.obterPagamentoPorId(1))
            .resolves
            .toEqual(pagamentoFake);
    });
    ...
    test('criarPagamento cria novo pagamento', async () => {
        ...
        jest.spyOn(Pagamento, 'create')
            .mockResolvedValue({ id: 1, ...novoPagamento });

        await expect(PagamentoService.criarPagamento(novoPagamento))
            .resolves
            .toEqual(expect.objectContaining({ id: 1, ...novoPagamento }));
    });
    ...
    test('atualizarPagamento atualiza pagamento existente', async () => {
        ...
        jest.spyOn(Pagamento, 'update')
            .mockResolvedValue([1]);

        jest.spyOn(Pagamento, 'findByPk')
            .mockResolvedValue({ id, ...dadosAtualizacao });

        await expect(PagamentoService.atualizarPagamento(id, dadosAtualizacao))
            .resolves
            .toEqual(expect.objectContaining({ id, ...dadosAtualizacao }));
    });

    test('excluirPagamento remove pagamento', async () => {
        jest.spyOn(Pagamento, 'destroy')
            .mockResolvedValue(1);

        await expect(PagamentoService.excluirPagamento(1))
            .resolves
            .not
            .toThrow();
    });
    ...
    test('aprovarPagamento atualiza situação para PAGO', async () => {
        ...
        await expect(PagamentoService.aprovarPagamento(1))
            .resolves
            .not
            .toThrow();
    ...
    test('reprovarPagamento atualiza situação baseado na data de vencimento', async () => {
        ...
        jest.spyOn(Pagamento, 'findByPk')
            .mockResolvedValue(pagamentoVencido);

        jest.spyOn(Pagamento, 'update')
            .mockResolvedValue([1]);

        await expect(PagamentoService.reprovarPagamento(1))
            .resolves
            .not
            .toThrow();
    ...
    test('gerarPagamentosMensaisManualmente gera pagamentos para usuários ativos', async () => {
        ...
        await expect(PagamentoService.gerarPagamentosMensaisManualmente(1))
            .resolves
            .not
            .toThrow();

        expect(Pagamento.create).toHaveBeenCalledWith(expect.objectContaining({
            usuarioId: 1,
            tipo: 'PIX',
            valor: 100.00,
            situacao: SituacaoPagamentoEnum.ABERTO

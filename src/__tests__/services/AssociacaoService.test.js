...
describe('AssociacaoService', () => {
    test('obterAssociacaoPorId retorna associação quando encontrada', async () => {
        ...
        await expect(AssociacaoService.obterAssociacaoPorId(1))
            .resolves
    ...
    test('obterTodasAssociacoes retorna lista de associações', async () => {
        ...
        jest.spyOn(Associacao, 'findAll')
            .mockResolvedValue(associacoesFake);

        await expect(AssociacaoService.obterTodasAssociacoes(filters, orderBy))
            .resolves
            .toEqual(associacoesFake);

        expect(Associacao.findAll)
    ...
    test('criarAssociacao cria nova associação com sucesso', async () => {
        ...
        jest.spyOn(Associacao, 'create')
            .mockResolvedValue(associacaoCriada);

        await expect(AssociacaoService.criarAssociacao(novaAssociacao, files))
            .resolves
            .toEqual(associacaoCriada);

        expect(Associacao.create)
            .toHaveBeenCalledWith(expect.objectContaining({
                ...novaAssociacao,
                logoUrl: 'logo.png',
                logoDeclaracaoUrl: 'logo_dec.png'
    ...
    test('atualizarAssociacao atualiza associação existente', async () => {
        ...
        jest.spyOn(Associacao, 'findByPk')
            ...
            .mockResolvedValueOnce({ ...associacaoExistente, logoUrl: 'novo_logo.png' });

        jest.spyOn(Associacao, 'update')
            .mockResolvedValue([1]);
        ...
        jest.spyOn(path, 'join')
            .mockReturnValue('/path/to/logo_antigo.png');

        await expect(AssociacaoService.atualizarAssociacao(id, dadosAtualizacao, files))
            .resolves
            .toEqual(expect.objectContaining({
                ...dadosAtualizacao,
    ...
    test('excluirAssociacao remove associação e seus arquivos', async () => {
        ...
        jest.spyOn(Associacao, 'findByPk')
            .mockResolvedValue(associacaoParaExcluir);

        jest.spyOn(Associacao, 'destroy')
            .mockResolvedValue(1);
        ...
        jest.spyOn(path, 'join')
            .mockReturnValue('/path/to/file.png');

        await AssociacaoService.excluirAssociacao(1);

        expect(Associacao.destroy)

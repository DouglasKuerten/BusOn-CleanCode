...
describe('PagamentoController', () => {
  ...
  test('obterPagamentoPorId retorna pagamento quando encontrado', async () => {
    ...
    jest
      .spyOn(PagamentoService, 'obterPagamentoPorId')
  ...
  test('obterTodosPagamentos retorna lista de pagamentos', async () => {
    ...
    jest
      .spyOn(PagamentoService, 'obterTodosPagamentos')
  ...
  test('criarPagamento cria novo pagamento', async () => {
    ...
    jest
      .spyOn(PagamentoService, 'criarPagamento')
    ...
    expect(res.json).toHaveBeenCalledWith(
      expect.objectContaining({ id: 1, ...novoPagamento }),
    );
    expect(next).not.toHaveBeenCalled();
  });
  ...
  test('atualizarPagamento atualiza pagamento existente', async () => {
    ...
    jest
      .spyOn(PagamentoService, 'atualizarPagamento')
    ...
    expect(res.status).toHaveBeenCalledWith(StatusCodes.OK);
    expect(res.json).toHaveBeenCalledWith(
      expect.objectContaining({ id: 1, ...dadosAtualizacao }),
    );
    expect(next).not.toHaveBeenCalled();
  });
  ...
  test('excluirPagamento remove pagamento', async () => {
    ...
    jest.spyOn(PagamentoService, 'excluirPagamento').mockResolvedValue();
  ...
  test('aprovarPagamento aprova pagamento', async () => {
    ...
    jest.spyOn(PagamentoService, 'aprovarPagamento').mockResolvedValue();
  ...
  test('reprovarPagamento reprova pagamento', async () => {
    ...
    jest.spyOn(PagamentoService, 'reprovarPagamento').mockResolvedValue();
  ...
  test('gerarPagamentosMensaisManualmente gera pagamentos', async () => {
    ...
    jest
      .spyOn(PagamentoService, 'gerarPagamentosMensaisManualmente')
      .mockResolvedValue();

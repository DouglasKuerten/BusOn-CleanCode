...
export const usuarioSchema = yup.object().shape({
    ...
    associacaoId: yup
        .number()
        .nullable(),
    dataEntradaAssociacao: yup
        .date()
        .nullable(),
    tipoAcesso: yup
        .string()
        .oneOf(Object.values(TipoAcessoEnum), 'Tipo de acesso inválido. Valores possíveis: ' + Object.values(TipoAcessoEnum).join(', '))
    ...
    diasUsoTransporte: yup
        .array()
        .of(yup.string().oneOf(Object.values(DiasSemanaEnum), 'Dia da semana inválido. Valores possíveis: ' + Object.values(DiasSemanaEnum).join(', ')))
        .nullable(),
    exigirRedefinicaoSenha: yup
        .boolean()
        .default(false)
});
